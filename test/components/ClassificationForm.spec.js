...
describe('ClassificationFo component test suite', () => {
  ...
  afterEach(() => {
    ...
    it('verify component renders with no errors', () => {
        const wrapper = shallow(<ClassificationForm { ...context } />);
    ...
    it('Verify Form.Check renders and handles change', () => {
      ...
      const wrapper = shallow(<ClassificationForm errors={[]} {...context} onChange={onChange} />)
    ...
    it('should select communties', () => {
      ...
      expect(groups.at(0).find(Form.Label).text()).toContain('Community')
      ...
      expect(wrapper.find(Form.Group).at(1).find(ValidatedInput).props().placeholder).toEqual('Select...')
    ...
    it('Verify that empty defaults render correctly', () => {
      const wrapper = shallow(<ClassificationForm errors={[]} {...context2} />)
