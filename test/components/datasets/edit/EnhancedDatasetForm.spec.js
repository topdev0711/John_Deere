...
const context = {
  ...
  usability: 0,
  toggles: {"jdc.business_application_enabled" : {enabled : true}}
}
...
describe('EnhancedDatasetForm component test suite', () => {
  ...
  describe('Input validation', () => {
    it('verify inputs are invalid', async () => {
      ...
      const wrapper = shallow(<EnhancedDatasetForm context={context} title="Register Dataset" />)
      wrapper.setState({
    ...
    it('verify inputs are valid', async () => {
      ...
      expect(validatedInputs.at(7).props().isInvalid).toEqual(false)
    })
    ...
    it('verify schemas are invalid', async () => {
      ...
      await wrapper.instance().validateForm()
      wrapper.update()
      const accordion = wrapper.find('#schemaAccordion')
      expect(accordion.props().items[0].body.props.errors.length > 0).toEqual(true)
    })
    ...
    it('verify classifications are invalid', async () => {
      ...
      await wrapper.instance().validateForm();
      wrapper.update();
      const container = wrapper.find('#classificationContainer');
      const errors = container.find(EnhancedClassificationForm).at(0).props().errors;
  ...
  describe('Delete DataSet Tests',() => {
    it('Display delete button when editing the existing dataset',() => {
      const wrapper = shallow(<EnhancedDatasetForm context={context} title="Register Dataset" isEditing={true} dataset={dataset}/>);
    ...
    it('Should not display delete button when creating new dataset',() => {
      const wrapper = shallow(<EnhancedDatasetForm context={context} title="Register Dataset" isEditing={false} />);
      const deleteButton = wrapper.find(Button).filterWhere(button => button.props().id === 'deleteDatasetButton');
  ...
  it('should initialize with given dataset', async () => {
    ...
    const wrapper = shallow(<EnhancedDatasetForm context={ context } dataset={initialDataset} title="Register Dataset"/>)
  ...
  it('verify component renders necessary selects and bindings', async () => {
    ...
    const application = wrapper.find(ValidatedInput).filterWhere(s => s.props().id === 'application')
  ...
  it('verify component renders classification blocks', () => {
    const wrapper = shallow(<EnhancedDatasetForm context={ context } title="Register Dataset"/>)
    const addButton = wrapper.find(AddClassificationLink)
    expect(addButton).toHaveLength(1)
    expect(wrapper.state().classifications).toHaveLength(1)
    addButton.at(0).simulate('click');
    addButton.at(0).simulate('click');
    expect(wrapper.state().classifications).toHaveLength(3)
  })
  ...
  it('verify component removes duplicate classifications when adding source datasets', async () => {
    ...
    const wrapper = shallow(<EnhancedDatasetForm context={ context } dataset={newDataset} title="Register Dataset"/>);
  ...
  it('verify component handles cancellation', async () => {
    ...
    const wrapper = shallow(<EnhancedDatasetForm isEditing={true} cancelAndUnlock={cancelAndUnlock} context={context} dataset={{...dataset, status: 'AVAILABLE', schemas: [], linkedSchemas: []}} onCancel={onCancel} title="Register Dataset"/>)
  ...
  it('Verify addSchemas adds schemas based on input parameter and overrides field ids and schema ids', async () => {
    const wrapper = shallow(<EnhancedDatasetForm context={context} dataset={detailedDataset} title="Register Dataset"  />)
  ...
  it('Verify onCancel in ConfirmationModal', async () => {
    ...
    act(() => {
      accordions.at(0).props().items[0].actions[0].handler()
    })
  ...
  it('Verify that page will handle EDL formatted dataset', async () => {
    ...
    const wrapper = shallow(<EnhancedDatasetForm context={context} dataset={constructBodyFormat} title="Register Dataset" />);
  ...
  it('Verify removeSchema works with new schema', async () => {
    const wrapper = shallow(<EnhancedDatasetForm context={context} dataset={dataset} title="Register Dataset"  />)
    await wrapper.instance().setInitialState(dataset);
    wrapper.instance().addSchema(null)
    expect(wrapper.state().schemas).toHaveLength(4)
  ...
  it('Verify schemas Accordion renders with correct items and can remove', async () => {
    ...
    expect(accordions.at(0).props().items).toHaveLength(3)
    expect(wrapper.state().modal).toBeUndefined
  ...
  it('should not set localstorage when mounting', () => {
    shallow(<EnhancedDatasetForm context={context} dataset={{...dataset, status: 'AVAILABLE'}} isEditing={true} title="Editing Dataset" />);
    expect(utils.setLocalStorage).toHaveBeenCalledTimes(0);
  })

  it('should not set localstorage when mounting and not editing', () => {
    shallow(<EnhancedDatasetForm context={context} dataset={{...dataset, status: 'AVAILABLE'}} isEditing={false} title="Editing Dataset" />);
    expect(utils.setLocalStorage).toHaveBeenCalledTimes(0);
  })
  ...
  it('should POST dataset when submitting', async () => {
    ...
    const wrapper = shallow(<EnhancedDatasetForm context={context} setDataset={setDataset} dataset={expectedDataset} isEditing={true} title="Editing Dataset" />);
  ...
  it('Should allow the user to enable data recovery', () => {
    const wrapper = shallow(<EnhancedDatasetForm context={context}/>);
  ...
  describe('localstorage tests', () => {
    it('should pass initial dataset and current state in the same format when saving', async () => {
      ...
      const wrapper = shallow(<EnhancedDatasetForm context={context} dataset={inputDataset} isEditing={true} />);
    ...
    it('localstorage should handle storage full error', async () => {
      ...
      const wrapper = shallow(<EnhancedDatasetForm context={context} dataset={detailedDataset} isEditing={true} title="Editing Dataset"/>);
      await wrapper.instance().setInitialState(detailedDataset);
