...
const Search = () => {
  const {query: {searchTerm: initialSearchTerm, size = defaultSize, ...initialFilters}, basePath} = useRouter();
  ...
  return (
    <div>
      ...
      <Row hidden={!showFilters}>
        <Col md={{span: 24}}>
          <SearchFilters key={hash(filters)} filters={filters} setFilters={setFilters} setShowFilters={setShowFilters} />
        </Col>
      </Row>
      <div>
