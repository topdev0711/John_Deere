...
import EnhancedClassificationForm from '../../EnhancedClassificationForm';
...
import AddClassificationLink from "./AddClassificationLink";
...
const visibilityOptions = [
  {value: VISIBILITY.FULL_VISIBILITY, label: 'All Users'},
  {value: VISIBILITY.NO_VISIBILITY, label: 'Custodian Only'},
];

export class EnhancedDatasetForm extends Component {
  state = {
    _isMounted: false,
    id: null,
    name: '',
    ...
    visibility: VISIBILITY.FULL_VISIBILITY,
  };

  isBusinessAppEnabled = this.props?.context?.toggles['jdc.business_application_enabled']?.enabled || false
  isVisibilityEnabled = this.props?.context?.toggles['jdc.custodian_visibility_flag']?.enabled || false
  ...
  getApplications = async () => {
    try {
      ...
      this.setState({ showAppLoaded: true });
    } catch (error) {
      console.log(error);
    }
  }
  ...
  isPublicDataset = () => {
    ...
    return !!this.state.classifications.find((c) => c?.gicp?.id === gicpPublicId);
  }

  shouldDisableGicpPublic = () => {
    return this.isPublicDataset() || this.state.classifications.length > 1;
  }
  ...
  handleChangeTag = tag => {
    this.setState({ application: !tag ? '' : tag.value });
  }
  ...
  governanceDetailChanged = (blockId, field, value) => {
    const updated = this.state.classifications.map(block => {
      ...
      const cleanedValue = (field === 'additionalTags') ? (value || []).map(v => v.label.trim()) : value;
  ...
  addGovBlock = () => {
    ...
    this.setState({
      classifications: [...classifications, { id: uuid.v4(), isNew: true }]
    });
  }
  ...
  handleSubmit = async () => {
    ...
    if (this.isBusinessAppEnabled && this.state?.selectData?.length > 0 && (!!!this.state.application || this.state.application === '')) {
      datasetErrors.push({ context: { key: 'application' }, details: "Application cannot be empty. Please select a valid value from the list or create one which fits your requirement." });
    }
    ...
    return (
      <>
        ...
        <ConfirmationModal
            ...
            onCancel={() => {
              this.setDeletionModal(null);
              this.setState({requestComments: ''})
            }}
            onAccept={(deleteModal || {
              onAccept: () => {
              }
            }).onAccept()}
        />
        <Modal size="lg" show={this.state.showDocsModal} onHide={this.handleMdPreview}>
          ...
          <Modal.Body className="markdown"
                      dangerouslySetInnerHTML={{__html: marked(this.state.mdPreview ? this.state.mdPreview : 'No preview to display.')}}></Modal.Body>
          <Modal.Footer><Button variant="secondary" onClick={this.handleMdPreview}>Close Preview</Button></Modal.Footer>
        </Modal>
        <Modal show={showApplicationModal} onHide={() => {
          this.handleShow(false)
        }} size="xl" className="modal-xl-90ht">
          <Modal.Header closeButton>
          ...
          <Modal.Body>
            <MyApplicationForm
                ...
                onSuccess={selectApplication => {
                  this.handleSuccess(selectApplication)
                }}>
            </MyApplicationForm>
          </Modal.Body>
        </Modal>
          ...
          <Card.Body>
            <Card.Text as="div">
              <Form>
                ...
                {this.isVisibilityEnabled && (
                  <Form.Group>
                    ...
                    <ValidatedInput
                        component={Select}
                        id="visibility"
                        onChange={(item) => this.handleChange('visibility', item.value)}
                        value={visibility ? visibilityOptions.find((vis) => vis.value === visibility) : null}
                        options={visibilityOptions}
                        isInvalid={datasetErrors.some(({context: {key}}) => key === 'visibility')}
                        invalidMessage="Select a visibility option"
                    />
                  </Form.Group>
                )}
                <Form.Group>
                ...
                <Form.Group>
                  ...
                  {this.isBusinessAppEnabled ?
                      <ValidatedInput
                          ...
                          noOptionsMessage={() => 'Applications will be loaded in a moment. Please wait ...'}
                          isInvalid={datasetErrors.some(({context: {key}}) => key === 'application')}
                          invalidMessage="Must select an application"
                          inline
                          isClearable={true}
                          isApplication={true}
                ...
                <div style={styles.classificationContainer} id='classificationContainer'>
                  {classifications.length > 0 &&
                      classifications.map((block, idx) => (
                          <EnhancedClassificationForm
                              key={block.id}
                              defaultValue={block}
                              gicpPublicDisabled={this.shouldDisableGicpPublic()}
                              showRemoveButton={idx > 0}
                              onRemove={this.setModal.bind(this, {
                                action: 'remove',
                                onAccept: this.removeGovBlock.bind(null, block.id)
                              })}
                  ...
                  <AddClassificationLink onClick={this.addGovBlock} isPublicDataset={this.isPublicDataset()}/>
                </div>
                <hr/>
                <Attachments
                    dataset={dataset}
                ...
                />
                <SourcesEdit key="dataset-sources" sources={sources} setSources={this.setSources}
                             datasetErrors={datasetErrors} showSourceError={showSourceError} setModal={this.setModal}/>
                    ...
                    <>
                      ...
                      <Form.Row>
                        ...
                        <span style={styles.add}>
                          <Button id="linkSchema" disabled={phase.name !== 'Enhance'}
                                  onClick={this.toggleSchemaSelector} size="sm" variant="outline-primary"><MdLink/> Link Schema</Button>&nbsp;&nbsp;
                          <Button id="addSchema" disabled={phase.name !== 'Enhance'} onClick={() => this.addSchema()}
                                  size="sm" variant="outline-primary"><MdAdd/> Add Schema</Button>
                        </span>
                        </Form.Group>
                      </Form.Row>
                            ...
                            header: (
                                <>
                                  <span style={{display: 'block'}}
                                        className="text-muted small"><b>Name:</b> <i>{schema.name || 'None'}</i></span>
                                  <span style={{display: 'block'}}
                                        className="text-muted small"><b>Version:</b> <i>{schema.version || 'None'}</i></span>
                                </>
                            ),
        ...
        <Toast
            hidden={!showAppLoaded}
            show={showAppLoaded}
            onClose={() => this.setState({showAppLoaded: false})}
            style={{
              position: 'fixed',
              top: 20,
              right: 20,
        ...
        >
          <Toast.Header>
            <strong className="mr-auto">Applications are loaded successfully!</strong>
          </Toast.Header>
          <Toast.Body>Please select the appropriate application for the dataset.</Toast.Body>
        </Toast>
...
const EnhancedDatasetFormComponent = props => <AppStateConsumer>
  {ctx => <EnhancedDatasetForm {...props} context={ctx} />}
</AppStateConsumer>;

export default EnhancedDatasetFormComponent;