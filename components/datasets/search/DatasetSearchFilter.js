...
export class DatasetSearchFilter extends React.Component {
  constructor(props) {
    super(props);
  ...
  getFilters = (referenceData, loggedInUser, hiddenFilters) => {
    ...
    const {roleType, phase, gicp, category, community, subCommunity, countriesRepresented, personalInformation, development, access, custodian, myDataset} = this.state;
  ...
  getLineageFilters = (hiddenFilters) => {
    const {servers, databases, tableNames, lineageFilters} = this.state;
    const filterOptions = [];
        ...
        <Card>
          <Card.Body style={styles.filterCard}>
            ...
            <div style={styles.right}>
              ...
              <span><Button size="sm" variant="primary" onClick={this.props.applySearch}>Apply</Button></span>
            </div>
          </Card.Body>
        </Card>
      </>
...
const DatasetSearchFilterComponent = React.forwardRef((props, ref) => (
  <AppStateConsumer>
    {ctx => <DatasetSearchFilter ref={ref} {...props} context={ctx} />}
  </AppStateConsumer>
))

export default DatasetSearchFilterComponent;
